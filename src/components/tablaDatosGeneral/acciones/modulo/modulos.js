import React, { useEffect, useState } from 'react';
import {
  Button,
  Modal,
  ModalHeader,
  ModalBody,
  ModalFooter,
  Tooltip,
} from 'reactstrap';

import createNotification from '../../../notificaciones/flotantes';
import Formulario from '../../../formularios/modulos/crud';
import {
  eliminarModulo,
  listarMenusModulo,
} from '../../../../services/modulos';
import { listarMenus } from '../../../../services/menu';
import Checklist from './checkList';

const AccionModulo = ({ cell, titulo, funcionListar }) => {
  const [accion, setAccion] = useState('ver');
  const [menus, setMenus] = useState([]);
  const [menusM, setMenusM] = useState([]);
  const [lista, setLista] = useState([]);

  const [modalBasic, setModalBasic] = useState(false);
  const [modalLarge, setModalLarge] = useState(false);
  const [modalAsignar, setModalAsignar] = useState(false);

  const [tooltipVerOpen, setTooltipVerOpen] = useState(false);
  const [tooltipActualizarOpen, setTooltipActualizarOpen] = useState(false);
  const [tooltipEliminarOpen, setTooltipEliminarOpen] = useState(false);
  const [tooltipAsignarOpen, setTooltipAsignarOpen] = useState(false);

  const funcionEliminar = (id) => {
    eliminarModulo(id)
      .then(() =>
        createNotification(
          'success',
          'Eliminación exitosa',
          `Se ha eliminado el modulo`,
          'filled'
        )
      )
      .catch(() =>
        createNotification(
          'error',
          'Error',
          `Error al eliminar modulo`,
          'filled'
        )
      );
  };
  const funcionCerrar = () => {
    setModalLarge(false);
  };
  const MagnetButton = () => {
    listarMenusModulo(cell.data[cell.row.index].id)
      .then((response) => setMenusM(response.data))
      .catch(() => setMenusM([]));
    setModalAsignar(true);
  };
  useEffect(() => {
    const estructura = [];
    for (let i = 0; i < menus.length; i += 1) {
      estructura.push({
        id: menus[i].id,
        nombre: menus[i].nombre,
        estado: false,
      });
    }
    if (menusM.length > 0) {
      for (let i = 0; i < estructura.length; i += 1) {
        for (let j = 0; j < menusM.length; j += 1) {
          if (estructura[i].id === menusM[j].id) estructura[i].estado = 'true';
        }
      }
    }
    setLista(estructura);
  }, [menusM]);

  useEffect(() => {
    listarMenus()
      .then((response) => {
        setMenus(response.data);
      })
      .catch((response) => console.log(response));
  }, []);

  return (
    <>
      <Button
        /* outline */
        id="boton-ver"
        color="primary"
        className="glyph-icon simple-icon-eye m-1"
        onClick={() => {
          setAccion('ver');
          setModalLarge(true);
        }}
        type="button"
      >
        <input type="hidden" />
      </Button>
      <Button
        /* outline */
        id="boton-actualizar"
        color="secondary"
        className="glyph-icon simple-icon-pencil m-1"
        onClick={() => {
          setAccion('actualizar');
          setModalLarge(true);
        }}
        type="button"
      >
        <input type="hidden" />
      </Button>
      <Button
        /* outline */
        id="boton-eliminar"
        color="danger"
        className="glyph-icon simple-icon-trash m-1"
        onClick={() => setModalBasic(true)}
        type="button"
      >
        <input type="hidden" />
      </Button>
      <Button
        /* outline */
        id="boton-Asignar"
        color="success"
        className="simple-icon-magnet m-1"
        onClick={MagnetButton}
        type="button"
      >
        <input type="hidden" />
      </Button>

      <Modal
        isOpen={modalLarge}
        size="lg"
        toggle={() => setModalLarge(!modalLarge)}
      >
        <div className="modal-header">
          <h5>
            {accion === 'actualizar' ? `Actualizar ${titulo}` : `Ver ${titulo}`}
          </h5>
          <button type="button" className="close" onClick={funcionCerrar}>
            X
          </button>
        </div>
        <ModalBody>
          {accion === 'actualizar' ? (
            <Formulario
              cell={cell}
              accion="actualizar"
              funcionCerrar={funcionCerrar}
              funcionListar={funcionListar}
            />
          ) : (
            <Formulario
              cell={cell}
              accion="ver"
              funcionCerrar={funcionCerrar}
              funcionListar={funcionListar}
            />
          )}
        </ModalBody>
      </Modal>
      <Modal isOpen={modalBasic} toggle={() => setModalBasic(!modalBasic)}>
        <ModalHeader>Eliminar {titulo}</ModalHeader>
        <ModalBody>
          <h6>¿Esta seguro de eliminar el registro?</h6>
        </ModalBody>
        <ModalFooter>
          <Button
            outline
            color="danger"
            onClick={() => {
              funcionEliminar(cell.data[cell.row.index].id);
              funcionListar();
              setModalBasic(false);
            }}
          >
            Sí
          </Button>{' '}
          <Button
            outline
            color="primary"
            onClick={() => {
              setModalBasic(false);
            }}
          >
            No
          </Button>
        </ModalFooter>
      </Modal>
      <Modal
        isOpen={modalAsignar}
        toggle={() => setModalAsignar(!modalAsignar)}
      >
        <ModalHeader>
          Agregar menu al modulo {cell.data[cell.row.index].nombre}
        </ModalHeader>
        <ModalBody>
          {menusM !== undefined && (
            <Checklist lista={lista} idModulo={cell.data[cell.row.index].id} />
          )}
        </ModalBody>
      </Modal>

      <Tooltip
        placement="buttom"
        isOpen={tooltipVerOpen}
        target="boton-ver"
        toggle={() => setTooltipVerOpen(!tooltipVerOpen)}
      >
        Ver
      </Tooltip>
      <Tooltip
        placement="buttom"
        isOpen={tooltipActualizarOpen}
        target="boton-actualizar"
        toggle={() => setTooltipActualizarOpen(!tooltipActualizarOpen)}
      >
        Actualizar
      </Tooltip>
      <Tooltip
        placement="buttom"
        isOpen={tooltipEliminarOpen}
        target="boton-eliminar"
        toggle={() => setTooltipEliminarOpen(!tooltipEliminarOpen)}
      >
        Eliminar
      </Tooltip>
      <Tooltip
        placement="buttom"
        isOpen={tooltipAsignarOpen}
        target="boton-Asignar"
        toggle={() => setTooltipAsignarOpen(!tooltipAsignarOpen)}
      >
        Asignar menu
      </Tooltip>
    </>
  );
};

export default AccionModulo;
