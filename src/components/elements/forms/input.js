...
const Input = (props) => {
  const normalForm = (Props) => {
    ...
    return (
      <Col xs={size}>
        <FormGroup {...attrForm}>
          {type === 'checkbox' ? (
            <input {...attrInput} {...register(name)} />
          ) : null}
          {labelText && <Label {...attrLabel}>{labelText}</Label>}
          {type === 'text' ? (
            <input {...attrInput} {...register(name, validation[name])} />
          ) : null}
          {type === 'number' ? (
            <input
              type="number"
  ...
  return <>{(show === 'false' && normalForm(props)) || showForm(props)}</>;
};
